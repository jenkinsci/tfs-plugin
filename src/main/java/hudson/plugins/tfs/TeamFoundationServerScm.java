...
public class TeamFoundationServerScm extends SCM {
    ...
    public boolean pollChanges(AbstractProject hudsonProject, Launcher launcher, FilePath workspace, TaskListener listener) throws IOException, InterruptedException {
        ...
        if (lastRun == null) {
            ...
            try {
                return (server.getProject(getProjectPath(lastRun), getCloakPaths(lastRun)).getDetailedHistoryWithoutCloakedPaths(
                            lastRun.getTimestamp(), 
                            Calendar.getInstance(),
                            getCloakPaths(lastRun)
                        ).size() > 0);
            } finally {
                server.close();
            }
    ...
    protected PollingResult compareRemoteRevisionWith(
        ...
        try {
            final ChangeSet latest = tfsProject.getLatestUncloakedChangeset(cloakPaths);
